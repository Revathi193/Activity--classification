...
class Application(tornado.web.Application):
    def __init__(self, assembler):
        handlers = [
            (r"/act_estimate", EstimateHandler, {'assembler': assembler}),
...
class EstimateHandler(tornado.web.RequestHandler):
    def initialize(self, assembler):
        ...
        self.data = ActivitypredictionData()
    ...
    def post(self):
        ...
        self.data.input_data = req_data

        # Execute assembler
        self.assembler.run(self.data)
        logging.info("Message: %s", self.data.output_data)
        self.write({"Activity Classification": self.data.output_data})
