...
def task_train():
    ...
    return {
        'actions': ["docker run --volume \"%s\":/app/output --volume \"%s\":/app/data %s python3 -m activityprediction --mode train" % (output_path, data_path, IMAGE)]
    }
...
def task_batch():
    ...
    return {
        'actions': ["docker run --volume \"%s\":/app/output --volume \"%s\":/app/data %s python3 -m activityprediction --mode batch" % (output_path, data_path, IMAGE)]
    }

def task_web():
    """Run web mode inside the container"""
    return {
        'actions': ["docker run -p 8080:8080  %s python3 -m activityprediction --mode web" % IMAGE]
    }
